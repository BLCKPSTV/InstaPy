...
insta_username = 'manaviszabina'
insta_password = 'AD1ejq5Sz'
...
def job():
	try:
		...
		session.set_use_clarifai(enabled=True)
    		...
    		session.set_comments([u'Party!!:thumbsup:', 'Where is this??', 'Nice photograph!'], media='Photo')
    		session.set_comments(['Great Video, nicely made!', u'BAMBAMBAM!! :sparkles:'], media='Video')
    		session.set_comments(['Nice shot! @{}', 'Couldnt have done it any better! @{}'], media='Photo')
    		session.set_comments(['Cool!', 'Awesome!', 'Raving!!', 'Super!', u'Sick party!!  :trollface:', u':raised_hands: Automation Collective (Social Flux) :raised_hands:'])
   		session.set_dont_include(['random_user'])
    		session.set_dont_like([ 'boring', 'girl', 'raciscme', 'pro trump'])
		session.set_user_interact(amount=5, random=True, percentage=50, media='Photo')

		 # session clarifai actions
		session.clarifai_check_img_for(['rave'], comment=True, comments=['Party!!', 'Aciiiiiidd', 'check my rave music out in a few weeks!'])

   		 # actions
    		session.like_by_tags(['rave', 'techno', 'party', 'natureone', 'kompass', 'dancing', 'underground' ], amount=50)
		session.like_by_users(usernames=['random_user'], amount=10, random=True)
		session.like_by_feed(amount=10, randomize=True)
		session.interact_user_followers(['dj', 'ravers', 'crazy'], amount=10, random=True)
